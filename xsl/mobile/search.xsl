<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:template name="navigation" match="//course">
    <ul class="breadcrumbs">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <li>
            <a href="{$relpath}mobile/search/course.html">Course Search</a>  
          </li>
          <li>
            /
          </li> 
  ...
  <xsl:template name="pdf">
    <li>
      <a href="{$relpath}{encode-for-uri(substring-before($url, 'html'))}pdf?dept={$dept_param_seq}">
        <img class="icon" alt="pdf" height="23" width="23" src="{$relpath}images/pdf.png"/>
      </a>
    </li>
  </xsl:template>
  <!-- 
  ...
  <xsl:template name="search_form">
    ...
    <form action="{$relpath}mobile/search/course.html" method="GET" class="form-horizontal" role="form">   
      <!-- hidden input fields -->
      ...
      <!-- Term -->
      <div class="form-group">
        <label> Fall 2014 (term)
          <input type="checkbox" name="fall" value="Y"/> 
        </label> 
        <br></br>
        <label> Spring 2015 (term)
  ...
  <xsl:template name="search_filter">
    ...
    <xsl:variable name="show_per_page" select="50" /> 
    ...
    <xsl:for-each-group select="$course_results" group-by=".">
      <!-- Display the results by couse group and number -->
      ...
      <xsl:sort  select="course_number/num_int"/>
          ...
          <ul class="pagination pagination-sm">
            <xsl:for-each select="1 to $number_of_pages">
              ...
              <xsl:if test="position() = 1">
                ...
                <xsl:when test="($current_page - 1) &#62; 0"> 
                  ...
                  <li>
                    <a href="{$relpath}mobile/search/course.html?{$clean_querystring}pid={$prev_position}">
                      <span>&#60;&#60;</span>
              ...
              <xsl:choose>
                <xsl:when test="$current_position = $current_page">
                  <li class="active">
                    <a class="active" href="{$relpath}mobile/search/course.html?{$clean_querystring}pid={$current_position}">
                ...
                <xsl:otherwise>
                  <li>
                    <a href="{$relpath}mobile/search/course.html?{$clean_querystring}pid={$current_position}">
                      <xsl:value-of select="$current_position"/>
                    </a>
                  </li>
                </xsl:otherwise>
            ...
            <xsl:choose>
              <xsl:when test="($current_page + 1) &#60; $number_of_pages"> 
                ...
                <li>
                  <a href="{$relpath}mobile/search/course.html?{$clean_querystring}pid={$next_position}">
                    <span>&#62;&#62;</span>
        ...
        <xsl:if test="position() &#60;= $range_max and position() &#62;= $range_min">  
          <a class="search-result-link" href="{$relpath}mobile/courses/course.html?catnum={@cat_num}" title="{title}">
    ...
    <hr></hr>
    <a href="{$relpath}mobile/search/course.html" class="btn btn-default course-search">Clear results</a>
  </xsl:template>  
</xsl:stylesheet>
